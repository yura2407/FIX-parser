...
public class ThreadTest {
    ...
    private final ArrayBlockingQueue<Map<String, Map<Integer, String>>> queue = new ArrayBlockingQueue<>(1_000_0000);

    private final FixParser fixParser = new FixParser();

    @Test
    void encodeDecodeTenMillionMessages() {
        for (int i = 0; i < 10; i++) {
            FixEncoder.encodeBinary(MESSAGES[i % MESSAGES.length]);
        }
    }

    @Test
    void encodeTenMlnMessages() {
        for (int i = 0; i < 10000000; i++) {
            FixEncoder.encodeBinary(MESSAGES[i % MESSAGES.length]);
        }
    }

    private void encodeDecodeVerify(String message, Encoding encoding) {
        byte[] customEncoded = FixEncoder.encodeBinary(message);
        byte[] asciiEncoded = message.getBytes(StandardCharsets.US_ASCII);
        assertEquals(
                fixParser.parseBinaryNonRepetitive(customEncoded, Encoding.CUSTOM),
                fixParser.parseBinaryNonRepetitive(asciiEncoded, Encoding.ASCII)
        );
    }
}
